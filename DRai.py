...
def process_batch_dialogue(client, dialogues: list, delimiter="-----"):
    ...
    prompt = (
        "HW2 change the data"
        "你是一位打擊教練對話分析專家，請根據以下編碼規則評估教練隊球員時的每一句話，\n"
        + "\n".join(ITEMS) +
...
def main():
    ...
    for start_idx in range(0, total, batch_size):
        ...
        if start_idx == 0:
            batch_df.to_csv(output_csv, index=False, encoding="utf-8")
        else:
            batch_df.to_csv(output_csv, mode='a', index=False, header=False, encoding="utf-8")
